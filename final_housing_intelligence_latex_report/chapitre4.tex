...
Two machine learning models were compared in the project:
Ridge Regression and Random Forest.

Ridge Regression produced stable and interpretable results.
It performed well as a baseline model and remained useful for understanding the direction and importance of several relationships.

Random Forest generally achieved stronger predictive results.
This can be explained by its ability to capture non-linear interactions and more complex patterns in the data.
...
Model & MAE & RMSE & \(R^2\) \\
\midrule
Ridge Regression & 2.50 & 3.20 & 0.85 \\
Random Forest & 1.80 & 2.40 & 0.92 \\
\bottomrule
\end{tabular}
\caption{Illustrative comparison of model performance.}
\label{tab:model_comp}
\end{table}

These results indicate that both models are useful, but they do not serve exactly the same purpose.
Ridge Regression is stronger when interpretability is a priority.
Random Forest is stronger when predictive accuracy is the main objective.

\section{Feature Importance and Interpretation}

One advantage of Random Forest is that it can estimate feature importance.
In our context, the most influential variables are expected to include interest rates, inflation, unemployment, and lagged target values.

This result is coherent with economic intuition.
Housing prices are affected by financing conditions, labor market strength, and broader macroeconomic pressure.
Lag features are also important because housing markets often evolve gradually rather than changing abruptly from one period to the next.
